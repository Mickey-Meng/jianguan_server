...
//        String loginName = TokenData.takeFromRequest().getLoginName();
    ...
    public PageInfo<Task> getTaskListByUserName(
        ...
        taskPageInfo.setTotal(totalCount);
    ...
    public List<FlowTaskVo> convertToFlowTaskList(List<Task> taskList) {
        ...
        for (Task task : taskList) {
            ...
            FlowEntryPublish entryPublish = flowEntryPublishMap.get(task.getProcessDefinitionId());
            if(Objects.nonNull(entryPublish)){
                flowTaskVo.setEntryId(entryPublish.getEntryId());
            }
...
//                FlowTaskComment save = list.stream().filter(flowTaskComment -> flowTaskComment.getApprovalType().equals("save")).collect(Collectors.toList()).get(0);
     ...
     */
    @Override
    public String getNextExecutor(String processInstanceId) {
        if (StringUtils.isBlank(processInstanceId)) {
            return "";
        }
        List<String> users = new ArrayList<>();
        List<FlowNextTaskVo> taskInfoVos = authMapper.getTasksByProcessInstanceId(processInstanceId);
        ...
        for (FlowNextTaskVo taskInfoVo : taskInfoVos) {
            String formKey = taskInfoVo.getFormKey();
            String groupType = JSON.parseObject(formKey).getString("groupType");
            boolean readOnly = JSON.parseObject(formKey).getBooleanValue("readOnly");
            String taskDefKey = taskInfoVo.getTaskDefKey();
            FlowNode flowElement = (FlowNode) bpmnModel.getFlowElement(taskDefKey);
            UserTask userTask = (UserTask) flowElement;
            // 当查询的任务执行人字段不为空,
            if (StringUtils.isNotBlank(taskInfoVo.getAssignee())) {
                users.add(taskInfoVo.getAssignee());
            } else {
                if (userTask.getName().equals(FlowConstant.START_USER_NAME_VAR)) {
                    users.add(processInstance.getStartUserId());
                } else {
                    getUserByGroupType(userTask, processInstance.getStartUserId(), users, groupType);
                }
            }
     ...
     */
    @Override
    public Object getProcessNodeExecutors(String processDefinitionId, String processInstanceId) {
...
//                                                users.add(expressionValue);
                                    } else {
                                        expressionValue = getValue(hvis, userTask.getAssignee());
                                        users.add(expressionValue);
                                    }
...
//                                                List<UserVo> userVos = userVoService.getUserByCodeOrGroupId(expressionValue);
                                    ...
                                    List<String> values = (List) getValue(hvis, inputDataItem, List.class);
...
//                                                });
                            ...
                            }else if (!userTask.getCandidateUsers().isEmpty()) {
                                users.addAll(userTask.getCandidateUsers());
                            }  else {
                                // 设置为岗位, 部门,等
//                                        List<String> u = getTaskAssigneeByFlow(userTask, processInstance.getStartUserId());
//                                        users.addAll(u);
                            }
                            result.put(userTask.getName(), users);
                        }
                    }
                }
    ...
    public ResponseBase deleteProcessInstanceByBusinessKey(String businessKey) {
        ...
        if (Objects.nonNull(historicProcessInstance)) {
            this.deleteProcessInstance(historicProcessInstance.getId());
            return ResponseBase.success();
        }
        return ResponseBase.error("未删除成功");
    }
    ...
    private List<String> getUserByGroupType(UserTask userTask, String startUserId, List<String> result, String groupType) {
        switch (groupType) {
            // 角色
            case FlowConstant.GROUP_TYPE_ROLE_VAR:
                // 设置的角色ID数组
                List<String> roleIdList = userTask.getCandidateGroups();
                result.addAll(authMapper.getUserNameByRoleId(StringUtils.join(roleIdList, ",")));
                break;
            case FlowConstant.GROUP_TYPE_DEPT_VAR:
                List<String> deptIdList = userTask.getCandidateGroups();
                result.addAll(authMapper.getUserNameByDeptId(StringUtils.join(deptIdList, ",")));
                break;
            case FlowConstant.GROUP_TYPE_POST:
                List<String> postIdList = userTask.getCandidateGroups();
                result.addAll(authMapper.getUserNameByPostId(
                        new QueryWrapper<String>().in("up.post_id", postIdList).eq("u.user_status", 0).eq("u.deleted_flag", 1))
                );
                break;
            case FlowConstant.GROUP_TYPE_DEPT_POST_LEADER:
                System.out.println("发起人部门领导");
                result.addAll(authMapper.getDeptLeaderUserName(startUserId));
                break;
            case FlowConstant.GROUP_TYPE_UP_DEPT_POST_LEADER:
                System.out.println("上级部门领导");
                result.addAll(authMapper.getUpDeptLeaderUserName(startUserId));
                break;
            case FlowConstant.GROUP_TYPE_USERS:
                System.out.println("候选组");
                // 获取的用户账号数组
                List<String> candidateUsers = userTask.getCandidateUsers();
                result.addAll(authMapper.getUserNameByLogin(new QueryWrapper<String>().in("login_name", candidateUsers).eq("user_status", 0).eq("deleted_flag", 1)));
                break;
            default:
                break;
