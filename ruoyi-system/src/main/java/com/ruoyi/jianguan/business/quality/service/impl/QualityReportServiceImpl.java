package com.ruoyi.jianguan.business.quality.service.impl;

import cn.hutool.core.io.IoUtil;
import cn.hutool.core.util.ObjUtil;
import cn.hutool.poi.excel.ExcelUtil;
import cn.hutool.poi.excel.ExcelWriter;
import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.ruoyi.common.enums.BimFlowKey;
import com.ruoyi.common.utils.MyPageUtil;
import com.ruoyi.jianguan.business.quality.domain.dto.QualityReportPageDTO;
import com.ruoyi.jianguan.business.quality.domain.dto.QualityReportSaveDTO;
import com.ruoyi.jianguan.business.quality.domain.entity.QualityReport;
import com.ruoyi.jianguan.business.quality.domain.vo.QualityReportDetailVo;
import com.ruoyi.jianguan.business.quality.domain.vo.QualityReportPageVo;
import com.ruoyi.common.core.domain.entity.FileModel;
import com.ruoyi.jianguan.business.quality.mapper.QualityReportMapper;
import com.ruoyi.jianguan.business.quality.service.QualityReportService;
import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import com.ruoyi.common.core.domain.object.ResponseBase;
import com.ruoyi.flowable.domain.dto.FlowTaskCommentDto;
import com.ruoyi.flowable.service.FlowStaticPageService;
import com.ruoyi.common.core.sequence.util.IdUtil;
import org.springframework.beans.BeanUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import javax.servlet.ServletOutputStream;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.util.Comparator;
import java.util.List;
import java.util.Map;
import java.util.Objects;

/**
 * 质量简报 服务实现类
 *
 * @author qiaoxulin
 * @date 2022-06-02
 */
@Service
public class QualityReportServiceImpl extends ServiceImpl<QualityReportMapper, QualityReport> implements QualityReportService {

    @Autowired
    private QualityReportMapper qualityReportMapper;

    @Autowired
    private FlowStaticPageService flowStaticPageService;

    /**
     * 新增或者更新质量简报数据
     *
     * @param saveDto
     * @return
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public ResponseBase addOrUpdate(QualityReportSaveDTO saveDto) {
        //属性copy
        QualityReport qualityReport = new QualityReport();
        BeanUtils.copyProperties(saveDto, qualityReport);
        //简报附件
        qualityReport.setReportAttachment(JSON.toJSONString(saveDto.getReportAttachment()));
        //整改内容附件
        qualityReport.setReplyPhotoAttachment(JSON.toJSONString(saveDto.getReplyPhotoAttachment()));
        //新增
        boolean isStartFlow = false;

        if (Objects.isNull(saveDto.getId())) {
            qualityReport.setId(IdUtil.nextLongId());
            //判断是否是草稿
            if (saveDto.getDraftFlag() == 1) {
                isStartFlow = true;
            }
        } else {
            //判断是否是草稿转为正式数据
            QualityReport qualityRepor = this.getById(saveDto.getId());
            if (qualityRepor.getDraftFlag() == 0 && saveDto.getDraftFlag() == 1) {
                isStartFlow = true;
            }
        }
        // 编辑操作不修改审批状态
        if(ObjUtil.isNull(saveDto.getId())) {
            qualityReport.setStatus(0);
        }
        //保存
        boolean saveOrUpdate = this.saveOrUpdate(qualityReport);
        //新增且保存成功
        if (saveOrUpdate && isStartFlow) {
            //发起流程
            String processDefinitionKey = BimFlowKey.质量简报.getName();
            String businessKey = qualityReport.getId().toString();
            //设置流程的审批人
            Map<String, Object> auditUser = saveDto.getAuditUser();
            if (auditUser.isEmpty()) {
                throw new RuntimeException("流程的审批人不能为空！");
            }
            //发起流程
            try {
                FlowTaskCommentDto flowTaskComment = new FlowTaskCommentDto();
                flowTaskComment.setApprovalType("save");
                flowTaskComment.setComment("发起质量简报申请");
                JSONObject taskVariableData = new JSONObject(auditUser);
                flowStaticPageService.startAndTakeUserTask(processDefinitionKey, flowTaskComment, taskVariableData, null, null, saveDto.getCopyData(), businessKey);
            } catch (Exception e) {
                log.error("流程启动失败！e=" + e.getMessage());
                throw new RuntimeException("流程启动失败！e=" + e.getMessage());
            }
        }
        return ResponseBase.success(saveOrUpdate);
    }

    /**
     * 通过id获取一条质量简报数据
     *
     * @param id
     * @return
     */
    @Override
    public QualityReportDetailVo getInfoById(Long id) {
        //查询
        QualityReport qualityReport = this.getById(id);
        if (Objects.isNull(qualityReport)) {
            return null;
        }
        //属性copy
        QualityReportDetailVo vo = new QualityReportDetailVo();
        BeanUtils.copyProperties(qualityReport, vo);
        //简报附件
        vo.setReportAttachment(JSONArray.parseArray(qualityReport.getReportAttachment(), FileModel.class));
        //整改内容附件
        vo.setReplyPhotoAttachment(JSONArray.parseArray(qualityReport.getReplyPhotoAttachment(), FileModel.class));
        return vo;
    }

    /**
     * 分页查询质量简报数据
     *
     * @param pageDto
     * @return
     */
    @Override
    public PageInfo<QualityReportPageVo> getPageInfo(QualityReportPageDTO pageDto) {
        //分页查询
        PageHelper.startPage(pageDto.getPageNum(), pageDto.getPageSize());
        List<QualityReportPageVo> pageVoList = qualityReportMapper.getPageInfo(pageDto);
        //非空
        if (Objects.nonNull(pageVoList) && !pageVoList.isEmpty()) {
            pageVoList.forEach(pageVo -> {
                if(pageVo.getStatus() == 0) {
                    pageVo.setStatusStr("审批中");
                } else if(pageVo.getStatus() == 1) {
                    pageVo.setStatusStr("已审批");
                }else {
                    pageVo.setStatusStr("已驳回");
                }
            });
        }
        return MyPageUtil.getPageInfo(pageVoList.stream()
                .sorted(Comparator.comparing(QualityReportPageVo::getCreateTime).reversed()), pageVoList.size(), pageDto.getPageSize(), pageDto.getPageNum());
    }

    /**
     * 质量简报导出
     *
     * @param pageDto
     * @param response
     */
    @Override
    public void export(QualityReportPageDTO pageDto, HttpServletResponse response) {
        //分页查询  页数暂定为5000
        pageDto.setPageSize(5000);
        PageInfo<QualityReportPageVo> pageInfo = this.getPageInfo(pageDto);
        //导出
        ExcelWriter writer = ExcelUtil.getWriter();
        writer.addHeaderAlias("buildSectionName", "施工标段");
        writer.addHeaderAlias("title", "简报名称");
        writer.addHeaderAlias("createTime", "发布时间");
        writer.addHeaderAlias("statusStr", "状态");
        writer.merge(3, "质量简报");
        writer.write(pageInfo.getList(), true);
        response.setContentType("application/vnd.ms-excel;charset=utf-8");
        String name = ("质量简报");
        response.setHeader("Content-Disposition", "attachment;filename=" + name + ".xls");
        ServletOutputStream out = null;
        try {
            out = response.getOutputStream();
            writer.flush(out, true);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            writer.close();
        }
        IoUtil.close(out);
    }
}
