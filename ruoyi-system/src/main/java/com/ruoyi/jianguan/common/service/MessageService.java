...
import com.ruoyi.jianguan.common.dao.ZjQualityEventDAO;
import com.ruoyi.jianguan.common.dao.ZjSafeEventDAO;
import com.ruoyi.jianguan.common.domain.dto.MessageUserDTO;
...
public class MessageService {
    ...
    public ResponseBase getSupervisorMessagePrompt(){
        ...
        List<ZjSafeEvent> delaySafeEvents = zjSafeEventDAO.getDelayByJianLiId(user.getUserId().intValue());
        List<ZjSafeEvent> rectificationSafeEvent = zjSafeEventDAO.getRectificationByJianLiId(user.getUserId().intValue());
        List<ZjQualityEvent> delayQualityEvents = zjQualityEventDAO.getDelayByJianLiId(user.getUserId().intValue());
        List<ZjQualityEvent> rectificationQualityEvent = zjQualityEventDAO.getRectificationByJianLiId(user.getUserId().intValue());
        List<Produceandrecode> processFilling = produceandrecodeDAO.getProcessByJianLiId(user.getUserId().intValue());
        ...
        //新增: 监理也可以查看待整改事件
        List<ZjSafeEvent> safeEvents = zjSafeEventDAO.getGQByUserId(user.getUserId().intValue());
        List<ZjQualityEvent> qualityEvents = zjQualityEventDAO.getGQByUserId(user.getUserId().intValue());
        userDTO.setZjQualityEventList(qualityEvents);
        userDTO.setZjSafeEventList(safeEvents);

        List<Integer> produceIds = produceandrecodeDAO.getProduceIdsByUserId(user.getUserId().intValue());
    ...
    public ResponseBase getConstructionMessagePrompt(){
        ...
        List<ZjSafeEvent> safeEvents = zjSafeEventDAO.getGQByUserId(LoginHelper.getUserId().intValue());
        List<ZjQualityEvent> qualityEvents = zjQualityEventDAO.getGQByUserId(LoginHelper.getUserId().intValue());
        List<Produceandrecode> processFilling = produceandrecodeDAO.getApprovedProcess(LoginHelper.getUserId().intValue());
        userDTO.setProcessFilling(processFilling);
        userDTO.setZjQualityEventList(qualityEvents);
        userDTO.setZjSafeEventList(safeEvents);
    ...
    public ResponseBase readProduce(Integer id){
        try {
            ...
            ProduceandrecodeUser user =
                    produceandrecodeDAO.getProduceUserByGidAndUserId(id, LoginHelper.getUserId().intValue());
            if (user == null){
                return new ResponseBase(200, "没有查到该用户对应的工序!");
            }
            produceandrecodeDAO.updateProduceUserState(id, LoginHelper.getUserId().intValue());
