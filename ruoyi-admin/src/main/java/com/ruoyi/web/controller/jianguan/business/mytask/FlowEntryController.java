...
//        FlowEntryVo flowEntryVo = FlowEntry.FlowEntryModelMapper.INSTANCE.fromModel(flowEntry);
    ...
    private FlowTaskExt buildTaskExt(UserTask userTask) {
        ...
        if (MapUtil.isNotEmpty(extensionMap)) {
            ...
            List<JSONObject> propertyList = this.buildPropertyListExtensionElement(extensionMap);
            if (CollUtil.isNotEmpty(propertyList)) {
                flowTaskExt.setPropertyListJson(JSON.toJSONString(propertyList));
            }
    ...
    private ResponseResult<TaskInfoVo> verifyAndGetInitialTaskInfo(FlowEntry flowEntry) throws XMLStreamException {
        ...
        if (firstTask instanceof UserTask) {
            ...
            if (MapUtil.isNotEmpty(extensionMap)) {
                ...
                taskInfoVo.setPropertyList(this.buildPropertyListExtensionElement(extensionMap));
            }
        } else {
            taskInfoVo = new TaskInfoVo();
    ...
    private List<JSONObject> buildPropertyListExtensionElement(Map<String, List<ExtensionElement>> extensionMap) {
        List<ExtensionElement> formVariableElements =
        ...
        List<JSONObject> resultList = new LinkedList<>();
        List<FlowKeyValueDto> lists = new ArrayList<>();
        for (ExtensionElement e : formVariableElements) {
            FlowKeyValueDto dto=new FlowKeyValueDto();
            dto.setName(e.getAttributeValue(null, "name"));
            dto.setVal(e.getAttributeValue(null, "value"));
            lists.add(dto);
        }
        for (FlowKeyValueDto variable : lists) {
            resultList.add((JSONObject) JSON.toJSON(variable));
        }
        return resultList;
    }
     ...
     * @return
     */
    public List<FlowEntry> getFlowEntryByPublishId(String newEntryPublishId) {
        //查询流程所有节点
        List<FlowEntry> flowEntries = flowEntryService.getFlowEntryByPublishId(newEntryPublishId);
        return flowEntries;
    }
}
